...
class PlayerInterface {
  ...
  constructor(hls) {
    ...
    this.hls.on(Events.LEVEL_LOADED, (event, data) => {
      ...
      var map = this.hls.levels.slice(0);
      var level = Object.assign({}, map[data.level]); //Need to deep copy the level, else we override a property ("sliding") that makes live playback crash
      if (level.details) {
  ...
  setBufferMarginLive (bufferLevel, type) {
    var MIN_BUFFER_LEVEL = 10;
    this.hls.bufferMarginLive = bufferLevel + MIN_BUFFER_LEVEL;
  }
