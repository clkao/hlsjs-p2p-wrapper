...
class PlayerInterface extends EventEmitter {
  ...
  constructor(hls, Events, onDispose, onAttach, onDetach) {
    ...
    function onLevelLoaded (event, data) {
      ...
      var map = this.hls.levels;

      if (this.lastTrackChangeLevel === undefined || this.currentLevel !== this.lastTrackChangeLevel) {
        this.lastTrackChangeLevel = this.currentLevel;
        this.emit('onTrackChange', 0, [{
          type: 'video',
    ...
    this.hls.on(Events.LEVEL_HANDLED, (event, data) => {
      onLevelLoaded.call(this, event, data)
    });
