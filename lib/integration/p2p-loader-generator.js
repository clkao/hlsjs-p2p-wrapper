...
const P2PLoaderGenerator = function (hlsjsWrapper) {
  ...
  return class P2PLoader extends XhrLoader {
    ...
    load(url, responseType, onSuccess, onError, onTimeout, timeout, maxRetry, retryDelay, onProgress = null, frag = null) {
      ...
      if (frag) {
        if (!hlsjsWrapper.hasSession()) {
          window.console.error('Agent is not existing yet');
          // Should not happen. Means we loaded a frag before the manifest, of there's a problem in the dispose sequence.
          throw new Error('Agent is not existing yet');
        }
    ...
    loadError (xhrEvent) {
      ...
      if (!this.stats.aborted) { // loadError WILL be called if the xhr was aborted. Don't exectue the following code: we DON'T want to retry downloading in this case
        if (this.stats.retry < this.maxRetry) {
          window.console.warn(`${status} while loading ${this.url}, retrying in ${this.retryDelay}...`);
          this.destroy();
          ...
          window.clearTimeout(this.timeoutHandle); // Timeout already cleared in abort() if the above condition is this.stats.aborted === true
          window.console.error(`${status} while loading ${this.url}` );
    ...
    loadInternal() {
      if (this.frag) {
        ...
        var segmentView = new SegmentView({sn: this.frag.sn, trackView});

        this.abortCallback = hlsjsWrapper.getAgent().getSegment({url: this.url, headers: headers, withCredentials: this.withCredentials} , {onSuccess: this.loadSuccess.bind(this), onError: this.loadError.bind(this), onProgress: this.loadprogress.bind(this)}, segmentView);
      } else {
        super.loadInternal();
      }
    }
