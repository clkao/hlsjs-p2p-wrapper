...
const P2PLoaderGenerator = function (hlsjsWrapper) {
    ...
    return class P2PLoader extends XhrLoader {
        ...
        loadError (xhrEvent) {
            ...
            if (!this.stats.aborted) { // loadError will be called if the xhr was aborted. Don't execute the following code: we DON'T want to retry downloading in this case
                if (this.stats.retry < this.maxRetry) {
                    console.warn(`${status} while loading ${this.url}, retrying in ${this.retryDelay}...`);
                    // FIXME: we don't want to call destroy() here because it will
                    //        set stats.aborted. thus we wont ever properly fail 
                    //        (actually getting into the else path of this if block and calling onError)
                    //this.destroy();
