...
let Hlsjs, StreamrootPeerAgentModule;
...
class HlsjsWrapperPrivate {
  ...
  constructor (hlsjsConstructor, peerAgentModuleConstructor) {
    ...
    Hlsjs = hlsjsConstructor; // If this is not defined when we need it we'll throw an error at that point
                              // We don't need to DI this constructor when using the `createSRModule` legacy method
    StreamrootPeerAgentModule = peerAgentModuleConstructor;
  }
  ...
  createPlayer (hlsjsConfig, p2pConfig, contentId = null) {
    var player = newPlayer.call(this, hlsjsConfig || {});
    player.on(Hlsjs.Events.MANIFEST_LOADING, () => {
        // Once the manifest is loading we know for sure that `url` will be defined
        startSession.call(this, player, hlsjsConfig, p2pConfig, player.url, contentId);
    });
    return player;
  }
  ...
  createSRModule (p2pConfig, hlsPlayer, hlsEventsEnum, contentId = null) {
    createPeerAgent.call(this, p2pConfig, hlsPlayer, hlsEventsEnum, null, contentId)
  }
...
*/
function startSession (hlsjsPlayer, hlsjsConfig, p2pConfig, contentUrl, contentId = undefined) {
  if (!p2pConfig || typeof p2pConfig !== 'object') {
    throw new Error('p2pConfig must be a valid config object');
  }

  var player = hlsjsPlayer || newPlayer.call(this, hlsjsConfig || {});

  createPeerAgent.call(this, p2pConfig, hlsjsPlayer, Hlsjs.Events, contentUrl, contentId);
 ...
 * @return {Hlsjs}
 */
function newPlayer (hlsjsConfig = {}) {
  if (!Hlsjs) {
  ...
  return new Hlsjs(defaults({}, getConfig.call(this), hlsjsConfig));
}
 ...
 *
 */
function createPeerAgent (p2pConfig, hlsjsPlayer, hlsEventsEnum, url = null, contentId = null) {
  if (hasSession.call(this)) {
  ...
  const playerBridge = new PlayerInterface(hlsjsPlayer, hlsEventsEnum, p2pConfig, onDispose.bind(this));
