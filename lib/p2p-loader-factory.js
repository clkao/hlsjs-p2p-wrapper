  ...
  return class P2PLoader extends XhrLoader {
    ...
    constructor(config) {
      ...
      if (config) {
        ...
        this.withCredentials = !!(config.xhr && config.xhr.withCredentials); 
        this.headers = config.xhr && config.xhr.headers ? config.xhr.headers : [];
      }
    }
    ...
    loadInternal() {
      if (this.frag) {
        ...
        if (this.xhrSetup) {
          throw new Error("xhrSetup setup is not usable with Streamroot version. Please contact Streamroot support"); //TODO: should we throw? xhrSetup can still be used for playlists and keys
        }

        var trackCoord = new TrackCoord({level: this.frag.level});
        var segmentCoord = new SegmentCoord({sn: this.frag.sn, trackCoord});

        this.abortCallback = streamrootWrapper.srModule.getSegment({url: this.url, headers: headers, withCredentials: this.withCredentials} , {onSuccess: this.loadSuccess.bind(this), onError: this.loadError.bind(this), onProgress: this.loadprogress.bind(this)}, segmentCoord);
      } else {
        super.loadInternal();
      }
    }
