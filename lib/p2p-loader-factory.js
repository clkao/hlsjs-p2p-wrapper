  ...
  return class P2PLoader extends XhrLoader {
    ...
    load(url, responseType, onSuccess, onError, onTimeout, timeout, maxRetry, retryDelay, onProgress = null, frag = null) {
      if (frag) {
        ...
        var trackCoord = new TrackCoord({level: frag.level});
        var segmentCoord = new SegmentCoord({sn: frag.sn, trackCoord});

        this.p2pSegment = streamrootWrapper.srModule.getP2PData(segmentCoord);
